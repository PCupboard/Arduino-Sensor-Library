...
void Keypad::keyMaping() {
    ...
    Serial.println("Mapping started, follow the instructions to map keypad correctly:");
    ...
    for (int i = 0; i < 12; i++) {
        ...
        while (!keyScanned) {           
            ...
            for (int currentPin = 0; currentPin < count; currentPin++) {
                ...
                // ---- Where the actual testing happens if pins are connected when button pressed in
                for (int testingPin = 0; testingPin < count; testingPin++) {
...
char Keypad::getKey(){
    ...
    for (int i = 0; i < 12; i++) {
        const int drivePin = keypadLayout[i].drivePin;
        const int readPin  = keypadLayout[i].readPin;
